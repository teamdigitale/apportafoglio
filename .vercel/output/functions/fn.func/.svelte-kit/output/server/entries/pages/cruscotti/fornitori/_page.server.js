...
import { a as caricaAvvisi } from "../../../../chunks/shared.js";
async function load({ locals }) {
  ...
  if (connstandard) {
    ...
    candidature.forEach(
      (c) => {
        c.fornitori.forEach((f) => {
          if (tipologieFornitori.indexOf(f.RecordType.Name) == -1)
            tipologieFornitori.push(f.RecordType.Name);
          if (nomiFornitori.indexOf(f.Denominazione_Soggetto_Realizzatore__c.toUpperCase()) == -1)
            nomiFornitori.push(f.Denominazione_Soggetto_Realizzatore__c.toUpperCase());
        });
      }
    );
    avvisi.forEach((a) => {
    ...
    for (let z = 0; z < misure.length; z++) {
      ...
      misure[z].conteggioFornitori.sort((a, b) => {
        return b.count - a.count;
      });
      misure[z].valoreFornitori.sort((a, b) => {
