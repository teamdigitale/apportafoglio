...
async function calcola(conn, result, rolling) {
  ...
  for (let z = 0; z < result.length; z++) {
    ...
    for (let y = 0; y < valltempimedi[z].length; y++) {
      let tm = Math.ceil((new Date(valltempimedi[z][y].Data_completamento_attivit__c) - new Date(valltempimedi[z][y].Data_Contrattualizzazione__c)) / (1e3 * 60 * 60 * 24));
      tm = tm < 0 ? 0 : tm;
  ...
  for (let z = 0; z < result.length; z++) {
    if (rolling === 0) {
      ...
      result[z].tempomediotecnico = Math.round(valltasks[z].filter((tt) => tt.Esito__c === "Positivo" || tt.Esito__c === "Negativo").reduce(
        (a, c) => a + Math.round(
          (new Date(c.LastModifiedDate) - new Date(c.CreatedDate)) / (1e3 * 60 * 60 * 24)
        ),
        0
      ...
      result[z].tempomediotecnicorolling = Math.round(valltasks[z].filter((tt) => tt.Esito__c === "Positivo" || tt.Esito__c === "Negativo").reduce(
        (a, c) => a + Math.round(
          (new Date(c.LastModifiedDate) - new Date(c.CreatedDate)) / (1e3 * 60 * 60 * 24)
        ),
        0
  ...
  if (rolling === 0) {
    for (let z = 0; z < result.length; z++) {
      ...
      result[z].tempomedioformale = Math.round(valltasksformali[z].filter((tt) => tt.Esito__c === "Positivo" || tt.Esito__c === "Negativo").reduce(
        (a, c) => a + Math.round(
          (new Date(c.LastModifiedDate) - new Date(c.CreatedDate)) / (1e3 * 60 * 60 * 24)
        ),
        0
    ...
    for (let z = 0; z < result.length; z++) {
      ...
      result[z].tempomedioformalerolling = Math.round(valltasksformali[z].filter((tt) => tt.Esito__c === "Positivo" || tt.Esito__c === "Negativo").reduce(
        (a, c) => a + Math.round(
          (new Date(c.LastModifiedDate) - new Date(c.CreatedDate)) / (1e3 * 60 * 60 * 24)
        ),
        0
...
async function load({ locals }) {
  ...
  if (connection) {
    ...
    {
      values[0].forEach((c) => {
        if (c.outfunds__FundingProgram__r.outfunds__Parent_Funding_Program__r.Name !== "1.2 Abilitazione e facilitazione migrazione al Cloud" && c.outfunds__FundingProgram__r.outfunds__Parent_Funding_Program__r.Name !== "1.4.1 Esperienza del cittadino nei servizi pubblici") {
