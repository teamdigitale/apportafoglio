...
export async function load({ locals }) {
    ...
    if (connstandard) {
        ...
        const avvisi = await promiseQuery(conn, `select Id, outfunds__Parent_Funding_Program__r.Id,Misura_Padre_1__c, Misura_Padre_2__c, outfunds__Total_Program_Amount__c, Total_Program_Amount_Padre_1__c, Total_Program_Amount_Padre_2__c, Name, outfunds__Status__c  from outfunds__Funding_Program__c 
        where outfunds__Parent_Funding_Program__c != null  order by CreatedDate desc`, MAX_FETCH);
        return {
            misure: values[0],
            finanziate: values[1],
            rinunciate: values[2],
