...
export async function load({ cookies}) {
    ...
    if (u) {
        ...
        const cc = promiseQuery(conn,`select Servizio__r.Anagrafica_Servizi__r.Id, Id, Name, Candidatura__r.Id, Candidatura__r.outfunds__Status__c, Candidatura__r.Misura__c, Candidatura__r.outfunds__Applying_Organization__r.Name,Candidatura__r.Regione__c, Candidatura__r.Codice_CUP__c  from Servizio_Richiesta__c  where Candidatura__c in (select Id from outfunds__Funding_Request__c where outfunds__Applying_Organization__r.Account_Manager__c = '`+u.idutentesf+`' or outfunds__Applying_Organization__r.Tech_Implementation_User__c = '`+u.idutentesf+`')  and Servizio__r.Anagrafica_Servizi__r.Id != null and Candidatura__r.outfunds__Status__c = 'FINANZIATA' and Selezionato__c = true`);
