...
export async function load({ locals }) {
    ...
    if (connstandard) {
        ...
        const qcand = promiseQuery(conn, `select outfunds__Applying_Organization__r.Account_Manager__c,outfunds__Applying_Organization__r.Account_Manager__r.Name,Id, OwnerId, IsDeleted, Name, CreatedDate, CreatedById, LastModifiedDate, LastModifiedById,  outfunds__Application_Date__c,  outfunds__Applying_Organization__c, outfunds__Awarded_Amount__c, outfunds__Awarded_Date__c, outfunds__Close_Date__c, outfunds__Closed_reason__c, outfunds__FundingProgram__c, outfunds__Geographical_Area_Served__c,  outfunds__Requested_Amount__c, outfunds__Requested_For__c, outfunds__Status__c, Ammissibilit__c, Base64PDF__c, Cluster__c, Codice_CUP__c, Consenso_Dichiarazioni_Inoltre__c, Consenso_Dichiarazioni__c, Data_Finanziamento__c, Data_Ricezione_CUP__c, Data_invio_candidatura__c,  Finestra_Temporale__c,  Last_Confirmed_Step__c, Misura__c, Regione__c,  Step_candidatura__c, Tipologia_Istat__c,  Contrattualizzazione_Fornitori__c, Data_conclusione__c, Descrizione_Progetto__c, Nome_Progetto__c, Stato_Progetto__c, CLP__c, Data_avanzamento_in_Liquidato__c, Data_Contrattualizzazione__c,  Last_Confirmed_Step_Progetto__c, Stato_contrattualizzazione__c, Data_Inizio_Formazione__c, Data_fine_formazione__c, Dichiarazione_Conformit_Servizi_Pubblic__c, Dichiarazione_Conformit_Sito_Internet__c, Modalita_Erogazione__c, Numero_Medio_Partecipanti__c, Numero_Ore_Formazione__c, Url_Servizi_Pubblici_Digitali__c, Url_Sito_Internet__c, Numero_task__c, Attivita_completata__c, Codice_Amministrativo__c,  Motivazione_Altro_Revoca_Progetto__c, Motivazione_Revoca_Progetto__c,  Progetto_Terminato__c,  rinuncia__c, Controllo_URL__c, Codice_Gestionale__c,   Data_avanzamento_step_5__c, Data_completamento_attivit__c, Data_esito_controlli_automatici__c, Descrizione_controlli__c,   Due_date_controlli__c, Esito_controlli__c,  Integrazione_PDND__c,   Tipologia_codice__c, Check_Data_Avanzamento_Step_5__c, Data_avanzamento_in_Liquidazione__c, Notifica_Liquidazione__c,     Data_Revoca_Decretata__c, Revoca__c, Ultimo_Esito_Conformit_Tecnica__c, Codice_misura_SAP__c, Conto_Contabilit_Speciale__c, Data_estrazione__c,        Esito_Audit__c, Motivazione_Audit__c, Predisposizione_progetto__c, Realizzero_intervento_senza_fornitori__c,    Pacchetto_Servizi__c,   Awarded_Amount_Padre_1__c, Awarded_Amount_Padre_2__c,      Tipo_Misura__c,   Numero_notifiche__c, Data_asseverazione_tecnica__c from outfunds__Funding_Request__c where outfunds__Applying_Organization__c in (select Id from Account where IsDeleted =false `+
        (locals.user.utentestandard.area?
            ` and Area_geografica__c = '`+locals.user.utentestandard.area+`') `
            :` and (Account_Manager__c = '` + locals.user.utentestandard.viewas + `' or Tech_Implementation_User__c = '` + locals.user.utentestandard.viewas + `'))`)+
        ` `);
