    ...
    if (connection) {
        ...
        */
        ...
        for(let z = 0; z<result.length; z++){
            ...
            and WhatId in (select Id from outfunds__Funding_Request__c where outfunds__FundingProgram__r.outfunds__Parent_Funding_Program__r.Name = '`+result[z].misura+`' and outfunds__Applying_Organization__r.Tipologia_Ente__c = '`+result[z].tipologiaente+`')`);
            const numerocandidaturetec = values[0].filter(c => c.Ultimo_Esito_Conformit_Tecnica__c==='Positivo' && result[z].misura===c.outfunds__FundingProgram__r.outfunds__Parent_Funding_Program__r.Name && result[z].tipologiaente===c.outfunds__Applying_Organization__r.Tipologia_Ente__c).length;
            const numerocandidatureform = values[0].filter(c => (c.Stato_Progetto__c==='LIQUIDATO'||c.Stato_Progetto__c==='IN LIQUIDAZIONE') && result[z].misura===c.outfunds__FundingProgram__r.outfunds__Parent_Funding_Program__r.Name && result[z].tipologiaente===c.outfunds__Applying_Organization__r.Tipologia_Ente__c).length;
