<script>
    ...
    import { regioni } from "../js/limits_IT_regions";
    import { aree_geografiche } from "../js/aree";
    let width = 800;
    $: height = width;
    const padding = 0.1;
    $: projection = d3
                ...
                <div class="card-body text-center" bind:clientWidth={width}>
                    {#each aree_geografiche as ag}
                        {#if selectedArea === "ALL" || selectedArea === ag.area}
                            ...
                            <div
                                class="chip chip-simple {selectedArea === '' ||
                                selectedArea === ag.area
                                    ? 'chip-primary'
                                    : ''}"
                                on:click={() => toggleArea(ag.area)}
                            >
                                <span class="chip-label">{ag.area}</span>
                            </div>
                        {/if}
                    {/each}
                    {#if selectedArea !== "ALL" && selectedArea !== ""}
                        {#each aree_geografiche.filter( (ag) => (selectedArea === "" || selectedArea === "ALL" ? true : ag.area === selectedArea), )[0].regioni as r}
                            ...
                            <!-- svelte-ignore a11y-no-static-element-interactions -->
                            <div
                                class="chip chip-simple {selectedRegion === r
                                    ? 'chip-primary'
                                    : ''}"
